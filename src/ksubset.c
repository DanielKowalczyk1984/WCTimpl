...
void ksubset_rec_generate(void *data,ksubset_rec *set,ulong kmin, ulong kmax, ulong rq, ulong nq, void (*visit)(const void *,const void *, ulong)) {
    ...
    set->kmin = kmin;
    set->kmax = kmax;
    if( set->kmin > set->kmax) CC_SWAP(set->kmin,set->kmax,temp);
    if( set->kmax > set->n) set->kmax = set->n;
    if(set->kmin >set->n) set->kmin = set->n;
...
void ksubset_next_rec(void *data,ksubset_rec *set,ulong d){
    ...
    {
        PCOND(1);
        for (ulong x=x0; x<=x1; ++x)  { set->rv[d] = x;  ksubset_next_rec(data,set,d+1); }
        PCOND(2);
    }
    else  // backward:
    {
        PCOND(2);
        for (ulong x=x1; x>=x0; --x)  { set->rv[d] = x;  ksubset_next_rec(data,set,d+1); }
        PCOND(1);
    }
    PCOND(3);
#undef PCOND
