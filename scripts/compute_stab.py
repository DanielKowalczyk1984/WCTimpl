...
class DataParser(argparse.ArgumentParser):
    ...
    def __init__(self):
        ...
        self.add_argument('-l', '--list', nargs='+', type=str,
                          ...
                          default=["tot_lb_root", "tot_build_dd",
                                   "tot_pricing",
                                   "nb_generated_col"])
        self.add_argument('-H', '--header', nargs='+', type=str,
                          help='list of all measurements',
                          default=["cpu LB root", "cpu build",
                                   "cpu pricing",
                                   "#col"])
...
class Summary(object):
    ...
    def compute(self):
        for row in self.reader:
            f = float(row['tot_cputime'])
            if re.match(self.p, row['NameInstance']) and f <= 200:
                self.overall["#opt"] += 1
                for it in self.lst:
    ...
    def print_to_screen(self):
        output = str()
        if self.cl == "all":
            output += "overall," + str(self.n) + "," + str(self.m)
        else:
            output += self.cl + "," + \
                str(self.n) + "," + str(self.m)
